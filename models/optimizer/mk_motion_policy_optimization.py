...
class MKMotionPolicyOptimizer(Optimizer):
    ...
    def collision_objective(self, x: torch.Tensor, data: Dict) -> torch.Tensor:
        """Compute collision-only objective for AMOGG.

        Args:
            x [torch.Tensor]: the denosied signal at current step, detached and requires grad.
            data [Dict]: data dict that provides original data.

        Return:
            torch.Tensor. Collision objective value to maximize.
        """
        ...
        x = x + 0.0 # only non-leaf nodes can calculate gradients
        _, O, _ = data['start'].shape
        x[:, 0:O, :] = data['start'].clone()

        B, L, D = x.shape

        trajs_norm = x
        trajs_unorm = MecKinova.unnormalize_joints(trajs_norm)
        ...
        T_aw = copy.deepcopy(data['T_aw'])
        sdf_norm_value = copy.deepcopy(data['sdf_norm_value'])
        scene_mesh_center = copy.deepcopy(data['scene_mesh_center'])
        scene_mesh_scale = copy.deepcopy(data['scene_mesh_scale'])

        collision_loss = 0
        trajs_unorm_flatten = trajs_unorm[:,:-1,:].reshape(-1, D)
        agent_pcs_a = self.sampler.sample(trajs_unorm_flatten)
        agent_pcs_w = transform_pointcloud_torch(
            agent_pcs_a,
            T_aw.unsqueeze(1).repeat(1, L-1, 1, 1).reshape(-1, 4, 4)
        )
        agent_pcs_w = agent_pcs_a.reshape(B, L-1, -1, 3)
        norm_agent_pcs_w = (
            agent_pcs_w - scene_mesh_center.unsqueeze(1).unsqueeze(1)
        ) * scene_mesh_scale.unsqueeze(1).unsqueeze(1).unsqueeze(1)

        for l in range(L-1):
            norm_agent_pc_w = norm_agent_pcs_w[:,l,:,:].reshape(B, -1, 3)
            agent_points_num = norm_agent_pc_w.shape[1]
            agent_sdf_batch = F.grid_sample(
                sdf_norm_value.unsqueeze(1),
                norm_agent_pc_w[:,:,[2,1,0]].view(-1, agent_points_num, 1, 1, 3),
                padding_mode='border',
        ...
        return (-1.0) * (self.collision_weight * collision_loss)
