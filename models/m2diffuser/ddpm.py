...
from models.m2diffuser.amogg import AMOGGSampler
...
class DDPM(pl.LightningModule):
    def __init__(self, eps_model: nn.Module, cfg: DictConfig, has_obser: bool, *args, **kwargs) -> None:
        ...
        amogg_cfg = cfg.sample.amogg if ('sample' in cfg and 'amogg' in cfg.sample) else None
        self.amogg_enabled = False
        self.amogg_use_variance = True
        self.amogg_task_scale = 1.0
        self.amogg_collision_scale = 1.0
        self.amogg: Optional[AMOGGSampler] = None
        if amogg_cfg is not None and amogg_cfg.get('enabled', False):
            self.amogg_enabled = True
            self.amogg_use_variance = bool(amogg_cfg.get('use_variance', True))
            self.amogg_task_scale = float(amogg_cfg.get('task_scale', 1.0))
            self.amogg_collision_scale = float(amogg_cfg.get('collision_scale', 1.0))
            self.amogg = AMOGGSampler(
                kappa=float(amogg_cfg.get('kappa', 10.0)),
                eps=float(amogg_cfg.get('eps', 1e-8)),
                detach_output=bool(amogg_cfg.get('detach_output', True)),
            )
    ...
    def _resolve_task_objective(self) -> Optional[Callable]:
        if self.planner is None:
            return None
        task_objective = getattr(self.planner, 'objective', None)
        return task_objective if callable(task_objective) else None

    def _resolve_collision_objective(self) -> Optional[Callable]:
        if self.optimizer is None:
            return None
        collision_objective = getattr(self.optimizer, 'collision_objective', None)
        if callable(collision_objective):
            return collision_objective
        optimize = getattr(self.optimizer, 'optimize', None)
        return optimize if callable(optimize) else None
    ...
    def p_sample(self, x_t: torch.Tensor, t: int, data: Dict, opt: bool=False, plan: bool=False) -> torch.Tensor:
        ...
        ## AMOGG only applies when both objectives are available.
        amogg_applied = False
        if self.amogg_enabled and self.amogg is not None and opt and plan:
            task_objective_fn = self._resolve_task_objective()
            collision_objective_fn = self._resolve_collision_objective()
            if task_objective_fn is not None and collision_objective_fn is not None:
                sigma_t = model_variance if self.amogg_use_variance else None
                guidance = self.amogg.compute_guidance(
                    x_prev_joint=model_mean,
                    data=data,
                    t=t,
                    T=self.timesteps,
                    task_objective_fn=task_objective_fn,
                    collision_objective_fn=collision_objective_fn,
                    sigma_t=sigma_t,
                    task_scale=self.amogg_task_scale,
                    collision_scale=self.amogg_collision_scale,
                )
                model_mean = model_mean + guidance
                amogg_applied = True

        if not amogg_applied:
